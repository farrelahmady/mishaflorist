...
<header id="header"
    class="fixed w-full flex justify-between px-5 py-6 z-50 top-0 {{ $visibleOnScroll ? '-translate-y-full' : '' }} lg:px-24 lg:py-5 transition-all">
    <a href="{{ route('home') }}">
    ...
    <nav class="hidden lg:flex items-center gap-6">
        ...
        @livewire('components.nav-link', ['href' => route('katalog'), 'active' => request()->routeIs('katalog'), 'text' => 'katalog'], key('katalog'))
    </nav>
    ...
    <nav class="flex justify-center lg:hidden">
        ...
        <label id="screen" for="hamburger-toggle"
            class="hidden fixed inset-0 h-screen bg-black bg-opacity-25 -z-10"></label>
